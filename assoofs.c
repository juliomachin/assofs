...
// Actualizar información persistente del superbloque cuando hay un cambio
void assoofs_sb_sync(struct super_block *sb)
{
...
int assoofs_sb_get_a_freeblock(struct super_block *sb, uint64_t *block) {
	...
	// 1. Obtenemos información persistente del bloque
	if (mutex_lock_interruptible(&assoofs_sb_lock)) {
	...
	// 2. Recorremos el mapa de bits en busca de un bloque libre (bit = 1)
	...
	*block = i ; // Escribimos el valor de i en la dirección de memoria indicada como segundo argumento en la función
	...
	// 3. Actualizar el valor de free_blocks y guardar cambios en superbloque
